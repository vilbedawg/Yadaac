...
TEST(StreamUtilsTest, FilterByValue) {
  ...
  auto stream =
      automaton.make_stream("ushers") | su::filter([](const match& m) { return m.value == 1; });
...
TEST(StreamUtilsTest, FilterByPosition) {
  ...
  daac automaton(patterns);
  auto stream = automaton.make_stream("abc") |
                su::filter([](const match& m) { return m.start == 0 && m.end >= 2; });
  auto results = collect_stream(stream);
...
TEST(StreamUtilsTest, TransformToCustomType) {
  ...
  auto stream = automaton.make_stream("abc") | su::transform([](const match& m) {
                  return custom_result{.position = m.start, .id = m.value};
                });
...
TEST(StreamUtilsTest, TransformToLength) {
  ...
  daac automaton(patterns);
  auto stream = automaton.make_stream("abc") |
                su::transform([](const match& m) -> size_t { return m.end - m.start; });
...
TEST(StreamUtilsTest, NonOverlappingDense) {
  ...
  auto results = collect_stream(stream);
  // First match: "a" at [0,1). Next "a" at [1,2) doesn't overlap. Next "aa" at [0,2) overlaps.
  // etc. Non-overlapping keeps first match, skips overlapping ones
...
TEST(StreamUtilsTest, FilterThenTake) {
  std::vector<std::string> patterns = {"a"};
  daac automaton(patterns);
  auto stream = automaton.make_stream("aaaaa") |
                su::filter([](const match& m) { return m.start >= 2; }) | su::take(2);
  auto results = collect_stream(stream);
  EXPECT_EQ(results.size(), 2U);
...
TEST(StreamUtilsTest, TripleChain) {
  ...
  auto stream = automaton.make_stream("aaaaaaaaaa")  // 10 a's
                | su::skip(2) | su::filter([](const match& m) { return m.start % 2 == 0; }) |
                su::take(3);
  auto results = collect_stream(stream);
  EXPECT_EQ(results.size(), 3U);
  EXPECT_EQ(results[0].start, 2U);
