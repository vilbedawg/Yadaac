...
TEST(BaseAllocatorTest, AllLabelsAllocation) {
  ...
  for (int i = 0; i < 256; ++i) {
    labels[i] = static_cast<uint8_t>(i);
  }
  ...
  for (int i = 0; i < 256; ++i) {
    ...
    EXPECT_TRUE(child_indices.insert(child).second) << "Duplicate child index for label " << i;
  }
...
TEST(BaseAllocatorTest, MixedAllocations) {
  ...
  for (int round = 0; round < 50; ++round) {
    ...
    auto label = static_cast<uint8_t>(round % 256);
    uint32_t base1 = allocator.allocate_base(label);
    uint32_t child1 = base1 ^ label;
    EXPECT_TRUE(all_children.insert(child1).second) << "Collision on single-label round " << round;
...
TEST(BaseAllocatorTest, StressMultipleBlocks) {
  ...
  for (int i = 0; i < count; ++i) {
    uint32_t base = allocator.allocate_base(label);
    uint32_t child = base ^ label;
    EXPECT_TRUE(child_indices.insert(child).second) << "Duplicate at allocation " << i;
  }
